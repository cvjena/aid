...
def get_dataset_queries(gt_dir, query_dir):
    ...
    for query_file in glob(os.path.join(query_dir, '*_query.txt')):
        ...
        if os.path.exists(label_file):
            ...
            with open(label_file) as lf:
                relevant = set(int(l.strip()) - 1 for l in lf if l.strip() != '')
            
            with open(query_file) as qf:
                query_imgs = [int(l.strip()) - 1 for l in qf if l.strip() != '']
            
            for qid in query_imgs:
                queries[qid] = { 'img_id' : qid, 'relevant' : relevant }
